   ...
   jQuery.post(
         'process_folio.php?check_delete_folio=1&folio_id='+folio_id, 
         {
          ...
          {  
            console.log(data);
            if(data == 0){
                    message('warning', folio_bi_huy , jQuery("#cuc-error"));
                    setTimeout((function() {
...
function check_max_value(op,status) {
    ...
    if (va > max) {
        ...
        message('warning', so_tien_lon_hon_tien_dich_vu, jQuery("#cuc-error"));
    } else {
...
//                location.replace(url);
            ...
            message('warning', mang_noi_bo, jQuery("#cuc-error"));
        }).always(function() {
...
function change_all_category(op) {
    ...
    if (hasFutureDate && !displayed_alert) {
        ...
        sendFutureDate = confirm(chuyen_tien_tuong_lai);
    }
    var idx = 0;
    quay.each(function() {
        idx++;
    ...
    jQuery(".submit-button").click(function() { 
        if(jQuery("#reservation_id").val() != 0 && jQuery("#reservation_id").val() != '' ){
            if (jQuery("#right_content").find(".active_service").length>0) {
                if (confirm(click_tao_folio)) 
                    save_folio();
                
            } else{
                message("warning",folio_rong,jQuery("#cuc-error"));
            }
        }else{
            message("warning",folio_khong_hop_le,jQuery("#cuc-error"));
        }
    });

    jQuery(".update-button").click(function() {
        if(jQuery("#reservation_id").val() != 0 && jQuery("#reservation_id").val() != '' ){
            if (confirm(click_cap_nhat_folio)) {
                save_folio();
            }
        }else{
            message("warning",folio_khong_hop_le,jQuery("#cuc-error"));
                ...
                if (info['ok'] == 0) {
                    if(r_r_id == jQuery("#room_change_right option:selected").text()){
                        ...
                        if (info['folio_id']) {
                            ...
                            console.log(rr_id);
                            var url = "?page=reservation&cmd=create_folio&id=" + id + "&r_r_id=" + r_r_id +"";
                            message('tsuccess', da_chuyen_sang_phong + jQuery("#room_change_right option:selected").text(), jQuery("#cuc-error"));
                            location.replace(url);
                        }
                        else {
                            console.log("3_sdsd");
