    ...
    var nhap = [];
    function show_room( callback ){ 
      var tmlurl = 'cache/config/config_room'+PORTAL_ID+'.js'; 
        jQuery.ajax({
           type: 'POST',
           url: tmlurl,
           dataType: 'json',
           success: function(data){
             jQuery.each(data,function(key,value){
              ...
              var tmp =  {"name":value.type_name,"id":value.lv_id};

              arr_room_level[value.name] = tmp;
    ...
    /*function list_room_type(){
     ...
     // Name:  load_list_traveller_type     
    // Author:      dungnn
    // Time:        30/09/2014
    // Parameter: 
    // Description: List souce
     function load_list_traveller_type(){
        var tmlurl = 'save_reservation_room.php?list_traveller_type=1&q=tmp';
        jQuery("#traveller_level_id").html('');
        jQuery.ajax({
           type: 'POST',
           url: tmlurl,
           dataType: 'json',
           success: function(data){
            if(data != null){
                jQuery.each(data,function(key,value){
                  if(value!='' && key != ''){ 
                      jQuery("#traveller_level_id").append('<option value="'+key+'">'+value+'</option>'); 
                  }   
                });
             }         
            },
            error: function(){
                load_list_traveller_type();
    ...
    jQuery('#mask').show();
          ...
          function show_remark_saved( callback ){   
    ...
    function auto_list_tour(){
        ...
        jQuery.ajax({
           ...
           success: function(data){
            ...
            error: function(){
                message('warning','Lỗi kết nối database',jQuery("#cuc-error"));
            }
        });
    }
          ...
          jQuery.ajax({
           ...
           success: function(data){
            ...
            if(data != null){
                jQuery.each(data,function(key,value){
                    if(value.rate != null){
                       tmp += '<option value="'+number_format(value.rate,0)+ '"><option>';
                        // jQuery("datalist#price").append('<option value="'+number_format(value.rate,0)+ '"><option>');
                    }
                });
                jQuery("datalist#price").append( tmp ); 
             }         
            },
            error: function(){
...
jQuery(".price_fomat").priceFormat({prefix: '', suffix: '', centsLimit: 0, centsSeparator: '.', thousandsSeparator: ',', allowNegative: true});
    ...
    function getBalance(){
            ...
            var total_room_final=parseInt(totalroom)
                                ...
                                -parseInt(foc)
                                -parseFloat(totalroom*discount_percent/100)
...
//            });
    ...
    // Description: List souce
	 ...
	 function load_list_pay(){
        var tmlurl = 'save_reservation_room.php?list_pay=1';
        jQuery("#payment_method_id").html('<option value="0">'+pay_type+'</option>');
        jQuery.ajax({
           type: 'POST',
           url: tmlurl,
           dataType: 'json',
           success: function(data){
            if(data != null){
                jQuery.each(data,function(key,value){
                  if(value!='' && key != ''){ 
                      jQuery("#payment_method_id").prepend('<option value="'+key+'">'+value+'</option>'); 
                  }   
                });
             }         
            },
     ...
     function load_list_card(){
        var tmlurl = 'save_reservation_room.php?list_card=1';
        jQuery("#list_card_id").html('<option value="0">'+card_type+'</option>');
        jQuery.ajax({
           type: 'POST',
           url: tmlurl,
           dataType: 'json',
           success: function(data){
            if(data != null){
              //jQuery("#list_card_id").append('');
                jQuery.each(data,function(key,value){
                  if(value!='' && key != ''){ 
                      jQuery("#list_card_id").prepend('<option value="'+key+'">'+value+'</option>'); 
                  }   
                });
             }         
            },
    ...
    function guestCard(){
        var url = '?page=reservation&cmd=guest_registration_card&portal=default&id=';
        url += r_r_id;
        window.open(url);
    }
