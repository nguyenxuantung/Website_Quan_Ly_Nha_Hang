...
jQuery(document).ready(function(){
		var row = '<tr>'
                   ...
                   // +'<td  class="text-right"><input type="text" id="lydo1" style="margin: 0px;"/></td>'                    
				+'</tr>';
		...
		function show_info_service(amount,date,room,invoice_id,list_sp){
	           ...
	           success: function(data){
	            ...
	            if(data != 0){   
	                jQuery(".folio_service").html( data  );
	                 	...
	                 	jQuery(".hideok").show();
	                  /*else{
	                 	jQuery(".header_confirm").html("Bạn có chắc chắn muốn xóa?");
	                 	jQuery(".showok").show();
	                 	jQuery(".hideok").hide();
	                 }*/
	             }else{
		 ...
		 function delete_service( ){
		      if (jQuery("#lydo").val().length>0) {
                ...
                alert(ban_phai_nhap_ly_do_huy_thanh_toan);
                return false;
            }		  
		 var reason = jQuery('#lydo').val();		  
			var tmlurl = 'process_folio.php?delete_service=1&type='+type+'&reason='+reason+'&invoice_id='+invoice_idok+'#'+ Math.random(); 
	        jQuery.ajax({
	           type: 'POST',
	           url: tmlurl,
	           dataType: 'json',
	           success: function(data){
	                ...
	                if( data== '1' ){
	                	...
	                	alert(dich_vu_da_duoc_tao_folio_khong_duoc_xoa);
	                }
	            },
            	error: function(){  
	            }
		...
		function delete_services(){
            ...
            var reason = jQuery('#lydo1').val();
			jQuery.post(
	        'process_folio.php?delete_services=1&type='+type+'&reason='+reason+'#'+ Math.random() , 
	        {
	          "data" : arr_convert 
	        }).done(function(data) 
		        {  
					...
					jQuery("#table_services tbody tr").each(function(){
						...
						var idm = btn.val();  
						 if(  typeof(idm) != 'undefined'){
							...
							tmp["folio_id"] 				= '';  
							//tmp["reason"]     				= jQuery("#reason_"+idm).html();
		...
		function show_info_service_checkbox(){ 
			...
			var arr_service_checkbox = JSON.stringify(arr_services);
			 console.log( arr_service_checkbox  );
			jQuery("#body_confirm_delete_checkbox").html("");
			if(  count_object(arr_services) == 0   ){
				jQuery(".header_confirms").html("Bạn chưa chọn dịch vụ");
		        	...
		        	jQuery.each(arr_service ,function(key,value){
						...
						console.log( key  ); console.log(value);
		        	 	if( value != null){
                            ...
                            //tmp = tmp.replace("[[REASON]]",value.reason);                                                        
